...
From the normality test, we can see that the data is not normally distributed (skew.2SE<1, kurt.2SE<1, Shapiro Wilk p-value>0.05). Thus, the data meets the first assumption for a parametric t-test. 
...
```
#### Conclusion 
The p-value > 0.05 indicates that there is no statistically significant relationship between reading_time and condition, for both the salient words and the word after. Therefore, we accept the null hypothesis, that there is no difference in the mean reading times in the two conditions of our reading experiment. 
