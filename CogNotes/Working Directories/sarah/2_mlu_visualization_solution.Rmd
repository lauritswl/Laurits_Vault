...
getwd() # you can first check where your current wd is
setwd(".")
...
mlu_data <- read_csv("mlu_clean.csv")
...
ggplot(mlu_data, aes(y=mlu, x=date)) +
  geom_point() +
  theme_minimal() +
  labs(
    title="Child and Mother MLU",
    subtitle="Over a 2-year period of conversations"
  )

# the same with nice formatting
mlu_df %>% 
  ggplot() + 
  aes(x=date, y=mlu) +
  geom_point() +
...
mlu_df %>% 
  ggplot() +
  aes(y=mlu, x=date, color=speaker) +
  geom_point() +
  theme_minimal() +
  labs(
    title="Child and Mother MLU",
