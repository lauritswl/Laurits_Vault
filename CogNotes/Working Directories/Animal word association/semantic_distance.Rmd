...
# Load them all in (e.g. using `ldply()`)
...
freq_data %>% 
    head(20) %>% ggplot(aes(x=word, y=n, fill=word)) + geom_bar(width = 0.75,  stat = "identity", colour = "black", size = 1) + coord_polar(theta = "x") + xlab("") + ylab("") + ggtitle("Word Frequency") + theme(legend.position = "none") + labs(x = NULL, y = NULL)
...
Trial-by-trial "reaction times" of word entries (how fast did you come up with the word?)
...
Trial-by-trial semantic distances (line plot for each row in the df)
Try to interpret the patterns
```{r}
laurits_dist %>% 
...
The relation between semantic distance and reaction time (x = distance, y = time)
...
Discuss whether the results support our hypotheses or not.
